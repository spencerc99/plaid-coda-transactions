...
def format_transactions_into_rows(bank, transaction_resp):
    ...
    if 'transactions' not in transaction_resp:
        print(
            f'No transactions found or errored out. resp: {transaction_resp}')
    for transaction in transaction_resp['transactions']:
        ...
        for k, col_id in key_to_column_id.items():
            ...
            col["value"] = format_none(
                coda_column_to_plaid_mapper[k](transaction))
            row.append(col)
...
def add_transactions(bank, payload):
    req = requests.post(
        ...
        json=format_transactions_into_rows(bank, payload),
        headers=headers,
    )
    print(req.text)
...
def get_last_transaction_date_for_bank(bank):
    ...
    """
    req = requests.get(
        f"{base_uri}/docs/{doc_id}/tables/{bank_table_id}/rows/{bank}",
        headers=headers)
    req.raise_for_status()
    resp = req.json()
